...
fn check_transparent_pixel_simd_alt_method(image: &[RGBA<u8>]) -> bool {
    // Isolate only the alpha channel.
    let pixel_alpha = Vec::from_iter(image.iter().map(|pixel| pixel.a));
...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("Check Pixel SIMD Alt Method", |b| {
        b.iter(|| check_transparent_pixel_simd_alt_method(black_box(image_bytes.as_rgba())))
    });
