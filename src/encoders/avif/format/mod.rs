...
use crate::encoders::avif::format::boxes::*;
...
impl Aviffy {
    ...
    fn make_boxes<'data>(
        ...
        let ispe_prop = ipco
            .push(IpcoProp::Ispe(IspeBox { width, height }))
            .ok_or(io::ErrorKind::InvalidInput)?;

        // This is redundant, but Chrome wants it, and checks that it matches :(
        let av1c_color_prop = ipco
            .push(IpcoProp::Av1C(Av1CBox {
                seq_profile: self
                    .min_seq_profile
        ...
        if let Some(alpha_data) = alpha_av1_data {
            ...
            let av1c_alpha_prop = ipco
                .push(boxes::IpcoProp::Av1C(Av1CBox {
    ...
    pub fn to_vec(
        ...
        let mut file = self
            .make_boxes(color_av1_data, alpha_av1_data, width, height, depth_bits)
            .unwrap();
        let mut out = Vec::new();
