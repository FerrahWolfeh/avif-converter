use super::constants::{ColorPrimaries, MatrixCoefficients, TransferCharacteristics};
use super::writer::{Writer, WriterBackend, IO};
...
impl AvifFile<'_> {
    ...
    fn write_header(&mut self, out: &mut Vec<u8>) -> io::Result<()> {
        if self
            ...
            .ipco
            .ispe()
            .is_none_or(|b| b.width == 0 || b.height == 0)
        {
    ...
    pub fn write<W: Write>(&mut self, mut out: W) -> io::Result<()> {
        ...
        self.mdat
            .write(&mut Writer::new(&mut IO(out)), &self.meta.iloc)
    }
}
